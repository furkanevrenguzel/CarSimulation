...
namespace VehicleBehaviour {
    ...
    public class WheelVehicle : MonoBehaviour {
        ...
        public float manualityThrottle = 0f;
        public float manualityTurn = 0f;
        ...
        void FixedUpdate () {
            ...
            if (isPlayer) {
                // Accelerate & brake
                if (throttleInput != "" && throttleInput != null && manualityThrottle == 0)
                {
                    throttle = GetInput(throttleInput) - GetInput(brakeInput);
                }else if(manualityThrottle != 0) {
                    throttle = manualityThrottle;
                }

                // Boost
                boosting = (GetInput(boostInput) > 0.5f);
                // Turn
                steering =  ( (manualityTurn != 0) ? manualityTurn : turnInputCurve.Evaluate(GetInput(turnInput)) ) * steerAngle;
                // Dirft
